...
var createConfig = function createConfig(settings, rawParentConfig) {
  var hash = settings.hash,
    _settings$filename = settings.filename,
    filename = _settings$filename === undefined ? [] : _settings$filename;

  var outputPath = _path2.default.join((0, _paths.getCacheDir)(settings.cacheDir), hash);
  ...
  var ownConfig = {
    ...
    plugins: [].concat(_toConsumableArray(settings.plugins || []), [
